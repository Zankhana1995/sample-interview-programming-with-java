...
public class ReturnNumberOfDuplicatesFromSortedArray {
    public static void main(String[] args) {
        int[] nums = {0, 0, 1, 1, 1, 2, 2, 3, 3, 4, 5, 5, 5, 5};
        // return number of Unique elements -> Output : 6
        int res = returnUniqueElementCount(nums);
        System.out.println(res);
    }

    private static int returnUniqueElementCount(int[] nums) {
        int count = 1;
        if (nums.length <= 1)
            return nums.length;
